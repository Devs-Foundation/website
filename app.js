 ...
 en:{brand:"Dev's Foundation",tagline:"One brain. Every model. Infinite memory.",
  ...
  PAGES:{
   ...
   solution:{t:"Solutions",intro:"Give every model the same permanent brain — a git repository. The pieces are deliberately simple and open.",items:[
    ...
    ["Optional local viewer","Our own Shared Consensus Brain viewer is just a window to see the brain as a graph. The system works with it turned off."],
   ...
   infra:{t:"Infra & security",intro:"No web surface, and automatic recovery. The infrastructure is designed to run headless and heal itself.",items:[
    ...
    ["SSH only, no web panel","Zero login, zero dashboard. Management is git over SSH or our local Shared Consensus Brain viewer window."],
 ...
 es:{brand:"Dev's Foundation",tagline:"Un cerebro. Todos los modelos. Memoria infinita.",
  ...
  PAGES:{
   ...
   solution:{t:"Soluciones",intro:"Esta es nuestra solución a todos esos problemas: darle a cada modelo el mismo cerebro permanente — un repositorio git compartido por todos. En vez de una libreta de bolsillo que se llena, una biblioteca entera sin límite; en vez de comprimir y alucinar, memoria que nunca olvida. Las piezas son deliberadamente simples y abiertas.",items:[
    ...
    ["Visor local, opcional","Nuestro propio visor Shared Consensus Brain es solo una ventana para ver el cerebro como grafo. El sistema funciona con él apagado — cada modelo ya lo tiene todo localmente."],
    ...
    ["Corre en cualquier máquina","Toda la stack es open-source — Git, Shared Consensus Brain, Markdown, n8n y modelos abiertos. Corre bien en cualquier máquina que cumpla los requisitos mínimos, sin lock-in ni dependencia de ninguna empresa. Y el conocimiento que produces hoy no se rehace mañana, porque queda."],
   ...
   infra:{t:"Infra y seguridad",intro:"Sin superficie web, y con recuperación automática. La infraestructura está hecha para correr headless y curarse sola.",items:[
    ...
    ["Solo SSH, sin panel web","Cero login, cero dashboard, cero vector de ataque web. La gestión es git por SSH o nuestro visor local Shared Consensus Brain."],
 ...
 fr:{brand:"Dev's Foundation",tagline:"Un cerveau. Chaque modèle. Mémoire infinie.",
  ...
  PAGES:{
   ...
   solution:{t:"Solutions",intro:"Voici notre solution à tous ces problèmes : donner à chaque modèle le même cerveau permanent — un dépôt git partagé par tous. Au lieu d'un carnet de poche qui se remplit, une bibliothèque entière sans limite ; au lieu de compresser et halluciner, une mémoire qui n'oublie jamais. Les pièces sont volontairement simples et ouvertes.",items:[
    ...
    ["Visualiseur local, optionnel","Notre propre visualiseur Shared Consensus Brain n'est qu'une fenêtre pour voir le cerveau comme un graphe. Le système fonctionne éteint — chaque modèle a déjà tout en local."],
    ...
    ["Tourne sur n'importe quelle machine","Toute la stack est open-source — Git, Shared Consensus Brain, Markdown, n8n et modèles ouverts. Elle tourne sur n'importe quelle machine qui remplit les prérequis minimaux, sans lock-in ni dépendance à une entreprise. Et le savoir produit aujourd'hui n'est pas à refaire demain, car il reste."],
   ...
   infra:{t:"Infra & sécurité",intro:"Aucune surface web, et récupération automatique. L'infrastructure est faite pour tourner headless et se soigner seule.",items:[
    ...
    ["SSH seulement, sans panneau web","Zéro login, zéro tableau de bord, zéro vecteur d'attaque web. La gestion se fait par git en SSH ou notre fenêtre locale Shared Consensus Brain."],
 ...
 de:{brand:"Dev's Foundation",tagline:"Ein Gehirn. Jedes Modell. Unendliches Gedächtnis.",
  ...
  PAGES:{
   ...
   solution:{t:"Lösungen",intro:"Das ist unsere Lösung für all diese Probleme: jedem Modell dasselbe dauerhafte Gehirn zu geben — ein von allen geteiltes Git-Repository. Statt eines Taschennotizbuchs, das sich füllt, eine ganze Bibliothek ohne Grenze; statt zu komprimieren und zu halluzinieren, ein Gedächtnis, das nie vergisst. Die Teile sind bewusst einfach und offen.",items:[
    ...
    ["Optionaler lokaler Viewer","Unser eigener Shared Consensus Brain Viewer ist nur ein Fenster, um das Gehirn als Graph zu sehen. Das System funktioniert ausgeschaltet — jedes Modell hat bereits alles lokal."],
    ...
    ["Läuft auf jeder Maschine","Der ganze Stack ist Open Source — Git, Shared Consensus Brain, Markdown, n8n und offene Modelle. Er läuft auf jeder Maschine, die die Mindestanforderungen erfüllt, ohne Lock-in und ohne Abhängigkeit von einer Firma. Und das heute erzeugte Wissen muss morgen nicht neu erzeugt werden, denn es bleibt."],
   ...
   infra:{t:"Infra & Sicherheit",intro:"Keine Web-Oberfläche und automatische Wiederherstellung. Die Infrastruktur ist darauf ausgelegt, headless zu laufen und sich selbst zu heilen.",items:[
    ...
    ["Nur SSH, kein Web-Panel","Kein Login, kein Dashboard, kein Web-Angriffsvektor. Die Verwaltung erfolgt per Git über SSH oder unseren lokalen Shared Consensus Brain Viewer."],
 ...
 zh:{brand:"Dev's Foundation",tagline:"一个大脑。每个模型。无限记忆。",
  ...
  PAGES:{
   ...
   solution:{t:"解决方案",intro:"这就是我们对所有这些问题的解决方案：给每个模型同一个永久大脑 —— 一个所有人共享的 git 仓库。不是会满的口袋笔记本，而是一整座没有上限的图书馆；不是压缩与幻觉，而是永不遗忘的记忆。各个部件都刻意保持简单与开放。",items:[
    ...
    ["本地查看器（可选）","我们自己的 Shared Consensus Brain 查看器只是把大脑看成图谱的一扇窗。系统在它关闭时照常运作 —— 每个模型本地都已有一切。"],
    ["没有网页界面","没有仪表板、没有登录、没有面板 —— 这意味着零网页攻击面。私密的永不离开本地大脑；只有方法是公开的。"],
    ["无限记忆","大脑没有上下文窗口。数年的工作、决策与所学始终可取，不会退化 —— 模型在需要时加载所需。"],
    ["能在任何机器上运行","整套技术栈都是开源的 —— Git、Shared Consensus Brain、Markdown、n8n 与开源模型。它能在任何满足最低要求的机器上运行，没有锁定，也不依赖任何公司。你今天产生的知识明天不必重做，因为它留了下来。"],
   ...
   infra:{t:"基础设施与安全",intro:"没有网页面，并具备自动恢复。基础设施为无头运行、自我修复而设计。",items:[
    ...
    ["只走 SSH，没有网页面板","零登录、零仪表板、零网页攻击向量。管理经由 SSH 上的 git，或我们本地的 Shared Consensus Brain 查看器。"],
 ...
 pt:{brand:"Dev's Foundation",tagline:"Um cérebro. Todos os modelos. Memória infinita.",
  ...
  PAGES:{
   ...
   solution:{t:"Soluções",intro:"Esta é a nossa solução para todos esses problemas: dar a cada modelo o mesmo cérebro permanente — um repositório git partilhado por todos. Em vez de um bloco de bolso que enche, uma biblioteca inteira sem limite; em vez de comprimir e alucinar, memória que nunca esquece. As peças são propositadamente simples e abertas.",items:[
    ...
    ["Visualizador local, opcional","O nosso próprio visualizador Shared Consensus Brain é apenas uma janela para ver o cérebro como grafo. O sistema funciona com ele desligado — cada modelo já tem tudo localmente."],
    ...
    ["Corre em qualquer máquina","Toda a stack é open-source — Git, Shared Consensus Brain, Markdown, n8n e modelos abertos. Corre bem em qualquer máquina que cumpra os requisitos mínimos, sem lock-in nem dependência de nenhuma empresa. E o conhecimento produzido hoje não se volta a gastar amanhã, porque fica."],
   ...
   infra:{t:"Infra & segurança",intro:"Sem superfície web, e com recuperação automática. A infraestrutura é feita para correr sem ecrã e curar-se sozinha.",items:[
    ...
    ["SSH only, sem painel web","Zero login, zero dashboard, zero vetor de ataque web. A gestão é git por SSH ou o nosso visualizador local Shared Consensus Brain."],
...
var NODES=[
 ...
 {id:"n8n",l:"n8n",g:"tool",r:12},{id:"git",l:"Git + SSH",g:"tool",r:12},{id:"md",l:"Markdown",g:"tool",r:11},{id:"obs",l:"Consensus Brain",g:"tool",r:11},
...
function buildHome(){
 ...
 var DIFF={
  en:{t:"What makes this different",i:"Each piece exists on its own elsewhere — the combination is what we haven't found in another system.",items:[["Collaborative consensus","The models debate and agree before acting — they are not orchestrated to obey a fixed pipeline."],["A persistent shared brain","Plain Markdown + git, growing and syncing across machines and over time."],["Self-hosted & vendor-agnostic","Your infrastructure, your files — swap models freely, depend on no one."],["An optional local dashboard","A local-only panel (127.0.0.1, no login) that shows the brain as a living graph and runs the consensus — agenda, debates, decisions, history. Off by default; no exposed web surface."]]},
  pt:{t:"O que nos distingue",i:"Cada peça existe isolada noutro lado — é a combinação que não encontrámos noutro sistema.",items:[["Consenso colaborativo","Os modelos debatem e chegam a acordo antes de agir — não são orquestrados a obedecer a um pipeline fixo."],["Um cérebro partilhado persistente","Markdown + git simples, que cresce e sincroniza entre máquinas e ao longo do tempo."],["Self-hosted e agnóstico ao fornecedor","A tua infraestrutura, os teus ficheiros — trocas de modelo à vontade, não dependes de ninguém."],["Um dashboard local opcional","Um painel só local (127.0.0.1, sem login) que mostra o cérebro como grafo vivo e gere o consenso — agenda, debates, decisões, histórico. Desligado por defeito; sem superfície web exposta."]]},
  es:{t:"Qué nos distingue",i:"Cada pieza existe por separado en otro lugar — es la combinación lo que no hemos encontrado en otro sistema.",items:[["Consenso colaborativo","Los modelos debaten y acuerdan antes de actuar — no están orquestados para obedecer un pipeline fijo."],["Un cerebro compartido persistente","Markdown + git simple, que crece y se sincroniza entre máquinas y a lo largo del tiempo."],["Autoalojado y agnóstico al proveedor","Tu infraestructura, tus archivos — cambias de modelo libremente, no dependes de nadie."],["Un panel local opcional","Un panel solo local (127.0.0.1, sin login) que muestra el cerebro como grafo vivo y gestiona el consenso — agenda, debates, decisiones, historial. Apagado por defecto; sin superficie web expuesta."]]},
  fr:{t:"Ce qui nous distingue",i:"Chaque élément existe séparément ailleurs — c'est la combinaison que nous n'avons pas trouvée dans un autre système.",items:[["Consensus collaboratif","Les modèles débattent et s'accordent avant d'agir — ils ne sont pas orchestrés pour obéir à un pipeline fixe."],["Un cerveau partagé persistant","Du Markdown + git simple, qui grandit et se synchronise entre les machines et dans le temps."],["Auto-hébergé et agnostique au fournisseur","Votre infrastructure, vos fichiers — changez de modèle librement, ne dépendez de personne."],["Un tableau de bord local optionnel","Un panneau uniquement local (127.0.0.1, sans login) qui montre le cerveau comme un graphe vivant et gère le consensus — agenda, débats, décisions, historique. Désactivé par défaut ; aucune surface web exposée."]]},
  de:{t:"Was uns unterscheidet",i:"Jeder Baustein existiert anderswo für sich — es ist die Kombination, die wir in keinem anderen System gefunden haben.",items:[["Kollaborativer Konsens","Die Modelle diskutieren und einigen sich, bevor sie handeln — nicht orchestriert, um einer festen Pipeline zu gehorchen."],["Ein persistentes gemeinsames Gehirn","Einfaches Markdown + git, das über Maschinen und Zeit hinweg wächst und synchronisiert."],["Selbst-gehostet & anbieterunabhängig","Deine Infrastruktur, deine Dateien — wechsle Modelle frei, hänge von niemandem ab."],["Ein optionales lokales Dashboard","Ein rein lokales Panel (127.0.0.1, ohne Login), das das Gehirn als lebendigen Graphen zeigt und den Konsens verwaltet — Agenda, Debatten, Entscheidungen, Verlauf. Standardmäßig aus; keine exponierte Web-Oberfläche."]]},
  zh:{t:"我们的独特之处",i:"每一项单独在别处都存在——我们在其他系统中没有找到的是这个组合。",items:[["协作式共识","模型在行动前讨论并达成一致——不是被编排去服从固定流水线。"],["持久化共享大脑","纯 Markdown + git，跨机器、随时间增长并同步。"],["自托管、不依赖供应商","你的基础设施、你的文件——自由切换模型，不依赖任何人。"],["可选的本地仪表板","一个仅本地面板（127.0.0.1，无需登录），将大脑显示为活的图谱并管理共识——议程、辩论、决策、历史。默认关闭；无暴露的网络攻击面。"]]}
 };
 var dfc=DIFF[LANG]||DIFF.en;
 var diffCards="";dfc.items.forEach(function(x){diffCards+='<div class="tcard"><h3>'+x[0]+'</h3><p>'+x[1]+'</p></div>';});
 var diffBlock='<div class="diffsec"><h2 class="diff-h">'+dfc.t+'</h2><p class="intro">'+dfc.i+'</p><div class="tabcards">'+diffCards+'</div></div>';
